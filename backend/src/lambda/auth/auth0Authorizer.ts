...
const jwksUri = 'https://dev-e-ihva0e.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const signingKeys = keys
    ...
    .map((key) => {
      return { kid: key.kid, nbf: key.nbf, publicKey: cert(key.x5c[0]) }
    });
...
export function cert(cert: string) {
